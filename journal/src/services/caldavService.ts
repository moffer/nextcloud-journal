...
import { CALDAV_BIRTHDAY_CALENDAR } from './consts.js'
import {CalendarHome} from "../types/cdav-library/calendarHome.type";

const clients = {}

const getClientKey = (headers) => JSON.stringify(headers)

const getClient: (header?: any) => DavClient = (headers = {}) => {
 ...
 */
const getCalendarHome: (headers?) => CalendarHome = (headers?) => {
	let client = getClient(headers);
	console.log('client', client);
	return client.calendarHomes[0]
}
